...
from .fractals import DEFAULT_SPECS, FractalSpec, generate_koch_curve, generate_sierpinski_triangle
from .regression import fit_scaling_relationship, summarize_windows

GENERATOR_MAP = {
    "generate_koch_curve": generate_koch_curve,
    "generate_sierpinski_triangle": generate_sierpinski_triangle,
}
...
class FractalExperiment:
    ...
    def _generate_points(
        ...
        generator = GENERATOR_MAP[spec.generator]
        if name == "koch":
            return generator(iterations=iterations, samples_per_segment=sample_density)
        return generator(iterations=iterations, samples_per_triangle=sample_density)

    def run(self, name: str, iterations: int, sample_density: int = 120) -> ExperimentResult:
        points = self._generate_points(name, iterations, sample_density)
        counts = box_count(points, self.epsilons)
        spec = self.specs[name]
        regressions = summarize_windows(counts, theoretical_dimension=spec.theoretical_dimension)
