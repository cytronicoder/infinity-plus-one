...
def fit_scaling_relationship(
    ...
    d_est = -slope
    abs_error = float("nan") if theoretical_dimension is None else abs(d_est - theoretical_dimension)
    rel_error = (
        float("nan")
...
def sliding_windows(length: int, window: int = 5) -> List[slice]:
    ...
    return [slice(start, start + window) for start in range(0, max(1, length - window + 1))]


def summarize_windows(df: pd.DataFrame, theoretical_dimension: float | None = None) -> pd.DataFrame:
    ...
    for name, sl in WINDOW_PRESETS.items():
        ...
        result = fit_scaling_relationship(
            subset["log_epsilon"].to_numpy(), subset["log_counts"].to_numpy(), theoretical_dimension
        )
        records.append(
            {
                "window": name,
