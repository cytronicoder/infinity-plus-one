...
def generate_koch_curve(iterations: int = 4, samples_per_segment: int = 120) -> np.ndarray:
...
def generate_sierpinski_triangle(iterations: int = 4, samples_per_triangle: int = 120) -> np.ndarray:
    ...
    triangles: List[np.ndarray] = [np.array([[0.0, 0.0], [1.0, 0.0], [0.5, base_height]], dtype=float)]
    for _ in range(iterations):
...
DEFAULT_SPECS: Sequence[FractalSpec] = (
    ...
    FractalSpec("sierpinski", "generate_sierpinski_triangle", math.log(3) / math.log(2)),
)
