...
def plot_residuals(counts: pd.DataFrame, output: Path) -> None:
    result = fit_scaling_relationship(counts["log_epsilon"].to_numpy(), counts["log_counts"].to_numpy())
...
def plot_iteration_accuracy(summary: pd.DataFrame, output: Path) -> None:
    fig, ax = plt.subplots(figsize=(6, 3.5))
    sns.lineplot(data=summary, x="iteration", y="abs_error", hue="fractal", marker="o", ax=ax)
...
def run_pipeline(output_dir: Path, max_iter: int = 6, sample_density: int = 120) -> None:
    ...
    for spec in DEFAULT_SPECS:
        for iteration in range(1, max_iter + 1):
            result = exp.run(spec.name, iterations=iteration, sample_density=sample_density)
            ...
            plot_log_log(result.counts, f"{spec.name.title()} n={iteration}", output_dir / f"{prefix}_loglog.png")
            plot_residuals(result.counts, output_dir / f"{prefix}_residuals.png")
            plot_window_estimates(result.regressions, output_dir / f"{prefix}_windows.png")

            fine_row = result.regressions[result.regressions["window"] == "fine"].iloc[0]
            summary_rows.append(
                {
                    "fractal": spec.name,
                    "iteration": iteration,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max-iter", type=int, default=6, help="Maximum iteration depth")
    parser.add_argument("--density", type=int, default=120, help="Samples per segment/triangle")
