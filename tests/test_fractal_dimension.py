...
from fractal_dimension.fractals import DEFAULT_SPECS, generate_koch_curve, generate_sierpinski_triangle
...
def test_dimension_trends_toward_theory():
    ...
    for n in range(2, 5):
        ...
        d_est = result.regressions.loc[result.regressions["window"] == "fine", "d_est"].iloc[0]
...
def test_resampling_stability():
    ...
    low_est = low_density.regressions.loc[low_density.regressions["window"] == "fine", "d_est"].iloc[0]
    high_est = high_density.regressions.loc[high_density.regressions["window"] == "fine", "d_est"].iloc[0]
